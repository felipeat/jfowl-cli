...
public class EntidadesMgr {
	...
	public static void definirRelacionamentos(List<Entidade> entidades) {
		...
		criarLigacoes(entidades);
		
	}

	public static void criarLigacoes(List<Entidade> entidades) {
		...
		for (Entidade e : entidades) {
			String uri = e.getURI().replace("http://dbpedia.org/resource/", "").replace("-", "_");
			Set<Entidade> rel = e.getRelacionados();
			if (rel.size() == 0) {
				gv.add(uri + ";");
			}
			else {
				for (Entidade e_rel : e.getRelacionados()) {
					String uri_rel = e_rel.getURI().replace("http://dbpedia.org/resource/", "").replace("-", "_");
					String opt1 = uri + " -- " + uri_rel + ";";
					String opt2 = uri_rel + " -- " + uri + ";";
					
					if (!gv.contains(opt1) && !gv.contains(opt2)) {
						for (int i = 0; i < e.qtdRelacionamentos(e_rel); i++) {
							gv.add(opt1);
						}
					}
				}
